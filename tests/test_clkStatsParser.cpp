 ...
 *   functioning of the ClkStatsParser class.
 *
...
//----------------------Testing Vars------------------------------------------
#define TEST_DATA_FILE "/usr/local/src/crsCorr_pre-1.1/data/clockstats.20100716"
const char TEST_DATA_PATH[] = "/usr/local/src/crsCorr_pre-1.1/data/";
...
bool testValues( const ClkStatsParser& parser );
...
int main() {
  ...
  // constructor with parameters
  {
    ...
    ClkStatsParser generic( &inputFile );
    paramCon = true;
    passFail( paramCon );
    cout << endl;
...
bool testValues( const ClkStatsParser& parser ) {
  ...
  for( int i = 0; i < ClkStatsParser::length; i++ ) {
    const string dataName = ClkStatsParser::tags[i].label;
    int dataType = ClkStatsParser::tags[i].type;
    string dataFileName;
    ...
    if( !dataFile.fail() ) {
      ...
      if( dataType == DATATYPE_INT ) {
        const DataSeries<int>* series =
            parser.getSeries<int>( &ClkStatsParser::tags[i] );
        ...
        const DataSeries<double>* series =
           parser.getSeries<double>( &ClkStatsParser::tags[i] );
        int len = series->getLength();
...
}// end bool testValues( const ClkStatsParser )
