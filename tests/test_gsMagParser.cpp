 ...
 *   functioning of the GsMagParser class.
 *
...
//----------------------Testing Vars------------------------------------------
#define TEST_DATA_FILE "/usr/local/src/crsCorr_pre-1.1/data/Gs/20100813_Gs_mag_1m.txt"
const char TEST_DATA_PATH[] = "/usr/local/src/crsCorr_pre-1.1/data/Gs";
...
bool testValues( const GsMagParser& parser );
...
bool testValues( const GsMagParser& parser ) {
  ...
  for( int i = 0; i < GsMagParser::length; i++ ) {
    const string dataName = GsMagParser::tags[i].label;
    int dataType = GsMagParser::tags[i].type;
    string dataFileName;
    ...
    if( !dataFile.fail() ) {
      ...
      if( dataType == DATATYPE_INT ) {
        const DataSeries<int>* series =
            parser.getSeries<int>( &GsMagParser::tags[i] );
        ...
        const DataSeries<double>* series =
           parser.getSeries<double>( &GsMagParser::tags[i] );
        int len = series->getLength();
...
}// end bool testValues( const GsMagParser )
