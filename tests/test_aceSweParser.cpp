...
//----------------------Testing Vars------------------------------------------
#define TEST_DATA_FILE "/usr/local/src/crsCorr_pre-1.1/data/ace/20100813_ace_swepam_1m.txt"
...
bool testValues( const AceSweParser& parser );
...
int main() {
  ...
  // constructor with parameters
  {
    ...
    AceSweParser generic( &inputFile );
    paramCon = true;
    passFail( paramCon );
    cout << endl;
...
bool testValues( const AceSweParser& parser ) {
  ...
  for( int i = 0; i < AceSweParser::length; i++ ) {
    string dataName = AceSweParser::tags[i].label;
    int dataType = AceSweParser::tags[i].type;
    string dataFileName;
    ...
    if( !dataFile.fail() ) {
      ...
      if( dataType == DATATYPE_INT ) {
        const DataSeries<int>* series =
            parser.getSeries<int>( &AceSweParser::tags[i] );
        ...
        const DataSeries<double>* series =
           parser.getSeries<double>( &AceSweParser::tags[i] );
        int len = series->getLength();
...
}// end bool testValues( const AceSweParser )
