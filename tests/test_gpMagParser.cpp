 ...
 *   functioning of the GpMagParser class.
 *
...
//----------------------Testing Vars------------------------------------------
#define TEST_DATA_FILE "/usr/local/src/crsCorr_pre-1.1/data/Gp/20100813_Gp_mag_1m.txt"
const char TEST_DATA_PATH[] = "/usr/local/src/crsCorr_pre-1.1/data/Gp";
...
bool testValues( const GpMagParser& parser );
...
bool testValues( const GpMagParser& parser ) {
  ...
  for( int i = 0; i < GpMagParser::length; i++ ) {
    const string dataName = GpMagParser::tags[i].label;
    int dataType = GpMagParser::tags[i].type;
    string dataFileName;
    ...
    if( !dataFile.fail() ) {
      ...
      if( dataType == DATATYPE_INT ) {
        const DataSeries<int>* series =
            parser.getSeries<int>( &GpMagParser::tags[i] );
        ...
        const DataSeries<double>* series =
           parser.getSeries<double>( &GpMagParser::tags[i] );
        int len = series->getLength();
...
}// end bool testValues( const GpMagParser )
